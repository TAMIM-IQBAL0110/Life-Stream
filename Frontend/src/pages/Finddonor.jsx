    ...
    const fetchDonors = async () => {
        try {
            ...
            const response = await axiosInstance.get(path, {
                params: { bloodGroup: blood, location }
            });
                ...
                <div className="max-w-6xl flex-1 justify-center mx-auto p-4 md:p-8">
                        ...
                        <div className="md:col-span-6">
                            ...
                            <div className="relative w-full rounded-lg">
                                <input 
                                    ...
                                    disabled={loading}
                                />
                                <span className="material-symbols-outlined text-red-500 absolute top-2 left-2">location_on</span>
                            </div>
                        </div>

                        <div className="flex items-end md:col-span-2 gap-2">
                            <button 
                                className="w-10 p-4 h-10 bg-red-600 hover:bg-[#ea2a33]/90 text-white font-bold rounded-[40px] flex items-center justify-center gap-2 transition-transform active:scale-95 shadow-lg shadow-[#ea2a33]/20 disabled:opacity-50" 
                    ...
                    {donors.length === 0 && !loading ? (
                        <div className="mt-8 text-center">
                            <h2 className="text-slate-600">No donors found. Try searching with different criteria.</h2>
                        </div>
                    ) : (
                        <div className="grid md:grid-cols-12 mt-7 gap-7">
                            <div className="col-span-7">
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    <span className="material-symbols-outlined text-red-600 bg-white p-1.5 rounded-lg ring-slate-200 ring-1">sort</span>
                                </div>
                                <div className="grid gap-4">
                                    {donors.map((item, index) => (
                                        <div key={index} className="bg-white rounded-xl grid p-4 shadow-sm ring-[0.5px] ring-slate-200 hover:ring-red-600">
                                            ...
                                            <h2 className="flex items-center gap-2 text-slate-500 text-sm"><span className="material-symbols-outlined text-[2px]">location_on</span>{item.location}</h2>
                                            <div className="my-2">
                                                <h2 className="text-sm text-slate-400">Last Donation</h2>
                                                <h3 className="text-sm font-semibold">{dateConvertion(item.lastDonatedTime)}</h3>
                                            </div>
                            ...
                            <div className="col-span-5 h-100 bg-slate-100 border-3 rounded-3xl border-white">
                                <h1 className="p-4">Map will be displayed here</h1>
                            </div>
                        </div>
                    )}
                </div>
